...
export function ChatContainer() {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] bg-white/40 backdrop-blur-2xl rounded-[40px] shadow-2xl shadow-primary/5 border border-white/20 overflow-hidden animate-fade-in">
      {/* ── Conversas Sidebar ── */}
      <aside className="w-[400px] border-r border-black/5 flex flex-col bg-gray-50/50 backdrop-blur-xl">
        <div className="p-8 pb-6">
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-3xl font-black tracking-tighter text-on-surface uppercase leading-none">Inbox</h1>
            <span className="bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-black">{rooms.length} THREADS</span>
          </div>
          <div className="relative group">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-on-surface/20 text-xl transition-colors group-focus-within:text-primary" size={18} />
            <input
              type="text"
              placeholder="Search conversations..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-16 pr-6 py-5 bg-white/80 border border-black/5 rounded-[24px] text-xs font-bold tracking-tight focus:ring-4 focus:ring-primary/5 outline-none transition-all shadow-sm"
            />
          </div>
        </div>

        <div className="flex-1 overflow-y-auto no-scrollbar py-2 space-y-1 px-4">
          {filteredRooms.length === 0 ? (
            <div className="p-20 text-center opacity-20 flex flex-col items-center select-none grayscale">
               <MessageSquare size={64} className="mb-4" />
               <p className="text-[10px] font-black uppercase tracking-widest">No connection established</p>
            </div>
          ) : (
            filteredRooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "w-full group relative p-5 rounded-[28px] flex items-center gap-5 transition-all",
                    isActive 
                ...
                >
                  <div className="relative shrink-0">
                    <div className={cn(
                      "w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-xl border transition-all",
                      isActive ? "bg-primary text-white border-primary shadow-lg shadow-primary/20" : "bg-white text-primary border-black/5"
                    )}>
                      {name.charAt(0).toUpperCase()}
                    </div>
                    {unread && (
                      <span className="absolute -top-1 -right-1 w-6 h-6 bg-primary text-white text-[10px] font-bold rounded-lg flex items-center justify-center border-2 border-white shadow-lg animate-bounce">
                        {room.unreadCount}
                      </span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex justify-between items-start mb-1">
                      <span className="font-bold text-base text-on-surface truncate tracking-tight">{name}</span>
                      <span className="text-[10px] text-on-surface/30 font-bold whitespace-nowrap pt-1">
      ...
      <main className="flex-1 flex flex-col bg-transparent relative overflow-hidden">
        <AnimatePresence mode="wait">
          {selectedRoomId && activeRoom ? (
            ...
            >
              <header className="h-28 px-10 flex items-center justify-between border-b border-black/5 bg-white/40 backdrop-blur-md z-20">
                <div className="flex items-center gap-6">
                  <div className="relative group transition-transform active:scale-95">
                    <div className="w-16 h-16 rounded-[24px] bg-primary/10 flex items-center justify-center font-bold text-2xl text-primary border border-primary/5 shadow-sm">
                    ...
                    {otherParticipant?.lastOnlineAt && new Date().getTime() - new Date(otherParticipant.lastOnlineAt).getTime() < 3 * 60000 && (
                      <span className="absolute -bottom-1 -right-1 w-6 h-6 bg-green-500 rounded-full border-4 border-white shadow-sm animate-pulse" />
                    )}
                  </div>
                  <div>
                    <h3 className="font-bold text-on-surface text-2xl tracking-tight leading-none mb-1.5">{getOtherName(activeRoom)}</h3>
                    <div className="flex items-center gap-2">
                      {otherParticipant?.lastOnlineAt && new Date().getTime() - new Date(otherParticipant.lastOnlineAt).getTime() < 3 * 60000 ? (
                        <span className="text-[10px] font-bold text-green-600 uppercase tracking-widest">Active Now</span>
                      ) : (
                        <span className="text-[10px] text-on-surface/40 font-bold uppercase tracking-wider flex items-center gap-1">
                          <Clock size={10} />
                          {otherParticipant?.lastOnlineAt ? `Last seen ${formatLastOnline(otherParticipant.lastOnlineAt)}` : 'Offline'}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-4">
                  <button className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/80 text-on-surface hover:bg-primary hover:text-white transition-all shadow-sm active:scale-90">
                    <Video size={22} />
                  </button>
                  <button className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/80 text-on-surface hover:bg-primary hover:text-white transition-all shadow-sm active:scale-90">
                    <Phone size={20} />
                  ...
                  <button
                    ...
                    className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/80 text-on-surface hover:bg-red-500 hover:text-white transition-all shadow-sm active:scale-90"
                  >
                    <Trash2 size={20} />
                  </button>
                  <button className="w-12 h-12 flex items-center justify-center rounded-2xl bg-white/80 text-on-surface hover:bg-surface-container-high transition-all shadow-sm active:scale-90">
                    <MoreVertical size={20} />
                  </button>
                </div>
              </header>

              <div className="flex-1 overflow-y-auto px-10 py-10 space-y-8 no-scrollbar scroll-smooth bg-gray-50/20">
                {messages.map((msg, idx) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={cn(
                        "group/msg flex flex-col gap-2 max-w-[80%]",
                        ...
                        {msg.deleted ? (
                          ...
                          <>
                            ...
                            {isMeetingLink ? (
                              ...
                              <div className={cn(
                                "p-6 rounded-[28px] text-sm leading-relaxed shadow-sm transition-all break-words whitespace-pre-wrap",
                                isMine 
                                  ? "bg-primary text-white shadow-primary/20 rounded-br-lg" 
              ...
              <footer className="p-10 bg-white/40 backdrop-blur-md border-t border-black/5">
                <form onSubmit={handleSendMessage} className="flex items-end gap-6 max-w-5xl mx-auto">
                   <div className="flex-1 relative group">
                      ...
                      <textarea 
                        className="w-full bg-white/80 backdrop-blur-sm rounded-[32px] border border-black/5 px-8 py-5 text-sm font-medium focus:ring-4 focus:ring-primary/5 transition-all outline-none resize-none no-scrollbar h-16 relative z-10"
                        placeholder="Share your clinical thoughts..."
                        value={inputText}
                        onChange={(e) => setInputText(e.target.value)}
                        onKeyDown={(e) => e.key === "Enter" && !e.shiftKey && (e.preventDefault(), handleSendMessage())}
                      />
                      <div className="absolute bottom-3 right-5 z-20 flex gap-1">
                         <button type="button" className="p-2 text-on-surface/20 hover:text-primary transition-colors active:scale-90">
                           <Paperclip size={20} />
                         </button>
                         <button type="button" className="p-2 text-on-surface/20 hover:text-primary transition-colors active:scale-90">
                           <Smile size={20} />
                   ...
                   <motion.button 
                    ...
                    className="w-16 h-16 flex items-center justify-center bg-primary text-white rounded-[24px] shadow-xl shadow-primary/20 hover:bg-primary-container disabled:opacity-30 transition-all"
                   >
                     <Send size={24} />
                   </motion.button>
                </form>
            ...
            >
               ...
               <h3 className="text-3xl font-black uppercase tracking-tight text-on-surface mb-4">Secure Channel</h3>
               <p className="text-sm font-medium max-w-[320px] leading-relaxed">
                 Select a health professional or provider to begin end-to-end encrypted communication.
               </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {showDeleteConfirm && (
           <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/20 backdrop-blur-2xl animate-fade-in p-6 text-center">
              ...
              >
                 ...
                 <h3 className="text-2xl font-black uppercase tracking-tight text-on-surface mb-4">Purge Thread?</h3>
                 <p className="text-sm font-medium text-on-surface-variant mb-10 leading-relaxed">
                   This action will irreversibly erase the entire communication record of this secure channel.
                 </p>
                 <div className="flex flex-col gap-3">
                    <button onClick={handleDeleteRoom} className="py-5 bg-red-500 text-white rounded-[24px] font-black uppercase tracking-widest text-[10px] hover:bg-red-600 shadow-xl shadow-red-500/20 active:scale-95 transition-all">
                      {deletingRoom ? "Purging..." : "Confirm Purge"}
                    </button>
